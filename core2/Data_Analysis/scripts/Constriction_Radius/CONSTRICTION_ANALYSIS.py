...
# -------------------------
parser = argparse.ArgumentParser(description="Constriction analysis of cylindrical membrane")
...
parser.add_argument("--frames", type=int, default=10, help="Number of frames to analyze")
parser.add_argument("--segment_width", type=float, default=20.0, help="Segment width in Å")
parser.add_argument("--bins", type=int, default=50, help="Number of histogram bins")
args = parser.parse_args()

traj_file = args.traj
tpr_file = args.tpr
Frame_Number = args.frames
segment_width = args.segment_width
Bin_Number = args.bins
...
Global_z_max_global = math.ceil(z_max_global)
print("Global_z_max_global:",Global_z_max_global)
...
filtered_segments_df.to_csv("mean_segment_df_V2.csv", index=False)
...
# Create position range from 0 to Global_z_max_global Å 
positions = np.arange(0, Global_z_max_global, segment_width)
...
for bin_index in unique_bins:
    ...
    if len(plot_df) > 0:
        ...
        # ---- Save the text file ----
        txt_filename = f"average_radius_vs_position_bin_{bin_index}.txt"
        plot_df[['position_Å', 'average_radius']].to_csv(
            txt_filename, sep='\t', index=False, float_format="%.4f"
        )
