...
# Process each bin
for bin_index in unique_bins[:1]:
...
# Save all radii for each bin and position into a text file
with open("all_radii_by_bin_and_position.txt", "w") as f:
    for _, row in result_df.iterrows():
        #f.write(f"{row['position_Å']}\n")
        #f.write("Radii:\n")
        if len(row["radius_measurements"]) > 0:
            f.write(" ".join(map(str, row["radius_measurements"])) + "\n")
        else:
            f.write("No radii found\n")
        f.write("\n")  # blank line between entries

print("✅ All radii saved to all_radii_by_bin_and_position.txt")
